...
def Teamuebersicht():
    ...
    return render_template('03_Teamübersicht/teams.html',teams=team)
...
def team_page(selected_team_name):
    ...
    for challenge in challenges_all:
        data = Session_db.scalars(Select(challenges_data).where(challenges_data.tea_id==selected_team.id,challenges_data.cmp_id==challenge.id).order_by(challenges_data.attempt_nr)).all()
    ...
    return render_template('03_Teamübersicht/team_page.html',team_name=selected_team_name,data=team_data)
...
def update_team_board(selected_team_name):
    ...
    team_data = []
    for challenge in challenges_all:
        raw_data = Session_db.scalars(Select(challenges_data).where(challenges_data.tea_id==selected_team.id,challenges_data.cmp_id==challenge.id).order_by(challenges_data.attempt_nr)).all()
